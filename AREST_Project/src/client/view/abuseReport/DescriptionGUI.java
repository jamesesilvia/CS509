...
public class DescriptionGUI extends JPanel implements CreateReportInterface, ViewReportInterface {
	...
	public void updatePanel(Report report, boolean redacted) {
		if(report.getInjuryList() != null) {
			injuryText.setText(report.getInjuryList());
		}
		if(report.getWitnesses() != null) {
			witnessText.setText(report.getWitnesses());
		}
		if(report.getCaregiverRel() != null) {
			relationshipText.setText(report.getCaregiverRel());
		}
		
		if(report.isIsOralReportFiled()) {
			chckbxDppcYes.setSelected(true);
		} else {
			chckbxDppcNo.setSelected(true);
		}
		
		if(report.isIsOralReportFiled() && report.getOralReportFiledTime() != null) {
			dppcCalledMonth.setText(report.getOralReportFiledTime().substring(0, 2));
			dppcCalledDay.setText(report.getOralReportFiledTime().substring(2, 4));
			dppcCalledHour.setText(report.getOralReportFiledTime().substring(4, 6));
			dppcCalledMinute.setText(report.getOralReportFiledTime().substring(6, 8));
		}
		
		if(report.isIsRiskToInvestigator()) {
			chckbxInvestRiskYes.setSelected(true);
			investRisk.setText(report.getRiskDescrToInvestigator());
		} else {
			chckbxInvestRiskNo.setSelected(true);
		}
	}
