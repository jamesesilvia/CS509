...
public class VictimGUI extends JPanel implements CreateReportInterface, ViewReportInterface {
	...
	private String[] comboSelectable = {"<Select One>", "Male", "Female"};
	...
	public void updatePanel(Report report, boolean redacted) {
		firstNameText.setText(report.victim.getFirstName());
		lastNameText.setText(report.victim.getLastName());
		addressTextArea.setText(report.victim.getAddress());
		phoneAreaCode.setText(report.victim.getPhoneNumber().substring(0, 3));
		phoneFirstThree.setText(report.victim.getPhoneNumber().substring(3, 6));
		phoneLastFour.setText(report.victim.getPhoneNumber().substring(6, 10));
		monthText.setText(report.victim.getDob().substring(0, 2));
		dayText.setText(report.victim.getDob().substring(2, 4));
		yearText.setText(report.victim.getDob().substring(4, 8));
		sexCombo.setSelectedItem(report.victim.getSex());
		maritalStatusText.setText(report.victim.getMaritalStatus());
		ageText.setText(report.victim.getAge());
		
		// Create a new string, don't want to overwrite the report when we replace the string.
		String disabilityString = report.victim.getDisability();
		for(int i=0; i<disabilityCbPanel.getComponentCount(); i++){
			if(disabilityCbPanel.getComponent(i) instanceof JCheckBox){
				JCheckBox cb = (JCheckBox) disabilityCbPanel.getComponent(i);
				if(report.victim.getDisability().contains(cb.getText())) 
				{
					disabilityString = disabilityString.replaceAll(cb.getText(), "");
					cb.setSelected(true);
				}
			}
		}
		disabilityString = disabilityString.trim();
		if(!disabilityString.equals("")){
			chckbxOtherspecify.setSelected(true);
			disabilityOtherTextField.setText(disabilityString);
		}
		
		
		String ethnicityString = report.victim.getEthnicity();
		for(int i=0; i<ethnicityCbPanel.getComponentCount(); i++){
			if(ethnicityCbPanel.getComponent(i) instanceof JCheckBox){
				JCheckBox cb = (JCheckBox) ethnicityCbPanel.getComponent(i);
				if(report.victim.getEthnicity().contains(cb.getText())) {
					ethnicityString = ethnicityString.replaceAll(cb.getText(), "");
					cb.setSelected(true);
				}
			}
		}
		ethnicityString = ethnicityString.trim();
		if(!ethnicityString.equals("")) {
			otherEthnicityCheckBox.setSelected(true);
			otherEthnicityTextField.setText(ethnicityString);
		}
		
		
		String comNeedsString = report.victim.getCommunicationNeeds();
		for(int i=0; i<comNeedsCbPanel.getComponentCount(); i++) {
			if(comNeedsCbPanel.getComponent(i) instanceof JCheckBox){
				JCheckBox cb = (JCheckBox) comNeedsCbPanel.getComponent(i);
				if(report.victim.getCommunicationNeeds().contains(cb.getText())) {
					comNeedsString = comNeedsString.replaceAll(cb.getText(), "");
					cb.setSelected(true);
				}
			}
		}
		comNeedsString = comNeedsString.trim();
		if(!comNeedsString.equals("")) {
			otherComNeedsCheckBox.setSelected(true);
			otherComNeedsTextField.setText(comNeedsString);
		}
