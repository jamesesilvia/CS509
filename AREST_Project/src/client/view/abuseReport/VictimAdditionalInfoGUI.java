...
public class VictimAdditionalInfoGUI extends JPanel implements CreateReportInterface, ViewReportInterface {
	...
	public Report updateReport(Report report, boolean redacted) {
		...
		victim.setDateOfLastIncident(txtMm.getText().concat(txtDd.getText().concat(txtYyyy.getText())));
	...
	public void updatePanel(Report report, boolean redacted) {
		...
		String currentlyServedByString = report.victim.getCurrentlyServedBy();
		for(int i=0; i<currentlyServedByPanel.getComponentCount(); i++) {
			if(currentlyServedByPanel.getComponent(i) instanceof JCheckBox) {
				JCheckBox myCb = (JCheckBox)currentlyServedByPanel.getComponent(i);
				if(report.victim.getCurrentlyServedBy().contains(myCb.getText())) {
					currentlyServedByString = currentlyServedByString.replaceAll(myCb.getText(), "");
					myCb.setSelected(true);
				}
			}
		}
		currentlyServedByString = currentlyServedByString.trim();
		if(!currentlyServedByString.equals("")) {
			otherServedText.setText(currentlyServedByString);
			otherSpecifyCb.setSelected(true);
		}

		String typeOfService = report.victim.getTypeOfService();
		for(int i=0; i<typeOfServicePanel.getComponentCount(); i++) {
			if(typeOfServicePanel.getComponent(i) instanceof JCheckBox) {
				JCheckBox myCb = (JCheckBox)typeOfServicePanel.getComponent(i);
				if(report.victim.getTypeOfService().contains(myCb.getText())) {
					typeOfService = typeOfService.replaceAll(myCb.getText(), "");
					myCb.setSelected(true);
				}
			}
		}
		...
		if(!typeOfService.equals("")) {
			otherServiceText.setText(typeOfService);
			otherServiceCb.setSelected(true);
		}

		if(report.victim.isAwarenessOfReport()) {
			yesCb.setSelected(true);
		} else {
			noCb.setSelected(true);
		}
		
		// This only allows one to be set. So we don't worry about multiple.
		for(int i=0; i<frequencyOfAbusePanel.getComponentCount(); i++) {
			if(frequencyOfAbusePanel.getComponent(i) instanceof JCheckBox) {
				JCheckBox myCb = (JCheckBox)frequencyOfAbusePanel.getComponent(i);
				if(report.victim.getFreqOfAbuse().contains(myCb.getText())) {
					myCb.setSelected(true);
				}
			}
		}
		
		txtMm.setText(report.victim.getDateOfLastIncident().substring(0, 2));
		txtDd.setText(report.victim.getDateOfLastIncident().substring(2, 4));
		txtYyyy.setText(report.victim.getDateOfLastIncident().substring(4, 8));
		
		
		String typesOfAbuse = report.victim.getTypesOfAbuse();
		for(int i=0; i<typesOfAbusePanel.getComponentCount(); i++) {
			if(typesOfAbusePanel.getComponent(i) instanceof JCheckBox) {
				JCheckBox myCb = (JCheckBox)typesOfAbusePanel.getComponent(i);
				if(report.victim.getTypesOfAbuse().contains(myCb.getText())) {
					typesOfAbuse = typesOfAbuse.replaceAll(myCb.getText(), "");
					myCb.setSelected(true);
				}
			}
		}
		typeOfService = typesOfAbuse.trim();
		if(!typesOfAbuse.equals("")) {
			otherServiceText.setText(typesOfAbuse);
			otherServiceCb.setSelected(true);
		}
